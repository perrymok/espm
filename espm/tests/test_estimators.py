...
from espm.estimators.surrogates import (
    diff_surrogate,
    smooth_l2_surrogate,
    smooth_dgkl_surrogate,
)
...
phases_dict = {
    "elts_dicts": [
        {"Fe": 0.54860348, "Pt": 0.38286879, "Mo": 0.03166235},
        {"Ca": 0.54860348, "Si": 0.38286879, "O": 0.03166235},
    ],
    "brstlg_pars": [{"b0": 5e-5, "b1": 3e-4}, {"b0": 7e-5, "b1": 5e-4}],
    "scales": [0.05, 0.05],
    "model_params": {
        "e_offset": 0.2,
        "e_size": 2000,
...
def generate_one_sample():
    ...
    model.generate_g_matr(
        ...
        elements=["Fe", "Mo", "Ca", "Si", "O", "Pt"],
        elements_dict={},
        ignored_elements=[],
    )
    G = model.G

    weights = generate_weights.generate_weights(
        "laplacian",
        misc_dict["shape_2d"],
        n_phases=len(phases_dict["elts_dicts"]),
        seed=misc_dict["seed"],
        size_x=10,
        size_y=10,
    )

    sample = generate_spim_sample(
        phases, weights, phases_dict["model_params"], misc_dict, seed=42, g_params={}
    )
