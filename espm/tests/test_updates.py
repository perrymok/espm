...
from espm.estimators.updates import (
    dichotomy_simplex,
    multiplicative_step_w,
    multiplicative_step_h,
    update_q,
    dichotomy_simplex_acc,
    multiplicative_step_hq,
)
from espm.estimators.updates import (
    estimate_Lipschitz_bound_h,
    estimate_Lipschitz_bound_w,
    gradW,
    gradH,
    proj_grad_step_h,
    proj_grad_step_w,
...
def make_step_a(
    x_matr, g_matr, p_matr, a_matr, mu_sparse=0, eps=log_shift, eps_sparse=1, mask=None
):
    """
    ...
    if mu_sparse != 0:
        ...
        if mask is None:
            ...
            # A update (masked)
            a_matr[n_mask] = (
                a_matr[n_mask] / (V[:, np.newaxis] + lambda_s)[n_mask] * U[n_mask]
            )

    else:
        # Lagragian multiplier
        lambda_s = dichotomy_simplex(a_matr * U, V[:, np.newaxis], 0, tol=dicotomy_tol)
...
def make_step_p(x_matr, g_matr, p_matr, a_matr, eps=log_shift):
    ...
    term2 = g_matr.sum(axis=0)[:, np.newaxis] @ a_matr.sum(axis=1)[:, np.newaxis].T
    # P update
...
def test_dicotomy_aq():
    def func_abc(x, a, b, minus_c):
        ...
        return (
            n_p * x
            + 2 * a
            + np.sum(-np.sqrt((b + x) ** 2 + 4 * a * minus_c) + b, axis=0)
        )
    ...
    np.testing.assert_allclose(
        func_abc(sol, a, b, minus_c), np.zeros([6]), atol=10 * tol
    )
...
def test_dichotomy_aq_constraint():
    ...
    def func_abc(x, a, b, minus_c, log_shift):
        return (
            np.sum(
                np.maximum(
                    np.sqrt((b + x) ** 2 + 4 * a * minus_c) - b - x, log_shift * 2 * a
                ),
                axis=0,
            )
            - 2 * a
...
def test_dicotom_aq2():
    ...
    for _ in range(its):
        ...
        np.testing.assert_allclose(
            func_abc(sol, a, b, minus_c), np.zeros([p]), atol=tol
        )

        log_shift = 0.1 / k
        sol = dichotomy_simplex_acc(
            a, b, minus_c, log_shift=log_shift, tol=tol, maxit=maxit
        )
        np.testing.assert_allclose(
            func_abc(sol, a, b, minus_c, log_shift=log_shift), np.zeros([p]), atol=tol
        )
...
def test_multiplicative_step_h():
    ...
    X = GP @ A
    np.testing.assert_allclose(
        np.sum(A, axis=0), np.ones([A.shape[1]]), atol=dicotomy_tol
    )

    Ap = multiplicative_step_h(
        X, G, P, A, simplex_H=False, mu=0, log_shift=0, epsilon_reg=1, safe=True
    )
    np.testing.assert_array_almost_equal(A, Ap)
    np.testing.assert_allclose(
        np.sum(Ap, axis=0), np.ones([Ap.shape[1]]), atol=dicotomy_tol
    )

    Ap = multiplicative_step_h(
        X, G, P, A, simplex_H=True, mu=0, log_shift=0, epsilon_reg=1, safe=True
    )
    ...
    Ap = multiplicative_step_h(
        X, G, P, A, simplex_H=True, mu=0, log_shift=0, epsilon_reg=1, safe=True, l2=True
    )
    np.testing.assert_allclose(A, Ap, atol=dicotomy_tol)
    ...
    for _ in range(10):
        ...
        Ap2 = make_step_a(
            X, G, P, A, mu_sparse=0, eps=log_shift, eps_sparse=1, mask=None
        )
        ...
        Ap2 = make_step_a(
            X, G, P, A, mu_sparse=1, eps=log_shift, eps_sparse=epsilon_reg, mask=None
        )
        np.testing.assert_array_almost_equal(Ap2, Ap)
        np.testing.assert_allclose(
            np.sum(Ap, axis=0), np.ones([Ap.shape[1]]), atol=dicotomy_tol
        ...
        Ap2 = make_step_a(
            X, G, P, A, mu_sparse=3, eps=log_shift, eps_sparse=epsilon_reg, mask=None
        )
        np.testing.assert_array_almost_equal(Ap2, Ap)
        np.testing.assert_allclose(
            np.sum(Ap, axis=0), np.ones([Ap.shape[1]]), atol=dicotomy_tol
...
def test_multiplicative_step_hq():
    ...
    for _ in range(10):
        ...
        Ap = multiplicative_step_hq(
            X, G, P, A, simplex_H=True, log_shift=log_shift, safe=True
        )
        Ap2 = make_step_a(
            X, G, P, A, mu_sparse=0, eps=log_shift, eps_sparse=1, mask=None
        )
...
def test_proj_step_h():
    ...
    def full_loss(X, G, W, H, log_shift, mu, lambda_L, L):
        return (
            KLdiv_loss(X, G @ W, H, log_shift=log_shift)
            + lambda_L * trace_xtLx(L, H.T)
            + log_reg(H, mu)
        )
