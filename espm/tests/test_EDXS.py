...
from espm.models.EDXS_function import (
    lifshin_bremsstrahlung,
    lifshin_bremsstrahlung_b0,
    lifshin_bremsstrahlung_b1,
)
import hyperspy as hs

x = np.linspace(0.2, 19, num=2000)
det_dict = {
    "detection": {"thickness": 450e-3, "density": 2.5},
    "layer1": {
        "thickness": 1e-6,
        "density": 2.3,
    ...
    "layer2": {"thickness": 3e-6, "density": 2.7, "elements_dict": {"Al": 1.0}},
}
...
model_parameters = {
    ...
    "E0": 200,
    "params_dict": {
        "Abs": {"thickness": 200.0e-7, "toa": 35, "density": 3.12},
        "Det": det_dict,
    },
}
...
def create_data():
    ...
    s.set_microscope_parameters(
        beam_energy=200, tilt_stage=0.0, azimuth_angle=0.0, elevation_angle=35.0
    )
...
def test_absorption():
    ...
    abs_corr = absorption_correction(
        x, thickness=1e-4, toa=35, density=5, elements_dict=elts
    )
...
def test_elts_dict_from_dict_list():
    dict_list = [
        {"chou": 1, "carottes": 2, "navet": 3},
        {"chou": 2, "oignons": 3, "navet": 3},
        {"orange": 6, "citron": 4, "poireau": 8},
    ]
...
def test_continuum_xrays():
    ...
    cont_x = ef.continuum_xrays(
        x,
        model_parameters["params_dict"],
        b0=b0,
        b1=b1,
        E0=E0,
        elements_dict={"Si": 1.0, "C": 0.2, "Bi": 3.2},
    )
...
def test_NMF_simplex():
    model = EDXS(**model_parameters)
    model.generate_g_matr(
        g_type="bremsstrahlung", elements=["Na", "Sr", "Ge", "Nb"], elements_dict={}
    )
    ...
    model = EDXS(**model_parameters)
    model.generate_g_matr(
        g_type="no_brstlg", elements=["Na", "Sr", "Ge", "Nb"], elements_dict={"Nb": 3.0}
    )
    inds = model.NMF_simplex()
    assert inds == [0, 1, 2, 4]
...
def test_NMF_update():
    model = EDXS(**model_parameters)
    model.generate_g_matr(
        g_type="no_brstlg", elements=["Na", "Sr", "Ge", "Nb"], elements_dict={}
    )
    temp_G = model.G.copy()
    ...
    with np.testing.assert_raises(AssertionError):
        np.testing.assert_array_equal(
            model.NMF_update(np.random.rand(6, 10))[:, -2:], temp_G[:, -2:]
        )
...
def test_generate_g_matr():
    ...
    G_brem = model1.G
    model2.generate_g_matr(
        g_type="identity", elements=elts_list, elements_dict={}, ignored_elements=[]
    )
    G_id = model2.G
    model3.generate_g_matr(
        g_type="no_brstlg", elements=elts_list, elements_dict={}, ignored_elements=[]
    )
    G_no_brstlg = model3.G
    model4.generate_g_matr(
        g_type="bremsstrahlung",
        elements=elts_list,
        elements_dict={"Ge": 3.0},
        ignored_elements=[],
    )
    G_brem_ge = model4.G
    model5.generate_g_matr(
        g_type="bremsstrahlung",
        elements=["Na", "Ge", "Nb"],
        elements_dict={"Ge": 3.0},
        ignored_elements=["Sr"],
    ...
    np.testing.assert_allclose(
        (G_brem_ge[:, 3] + G_brem_ge[:, 2]) * model4.norm[0, 0],
        G_brem[:, 2] * model1.norm[0, 0],
    )
...
def test_G_bremsstrahlung():
    ...
    B = ef.G_bremsstrahlung(
        model.x, model.E0, model.params_dict, elements_dict=elts_dict
    )
