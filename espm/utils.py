    ...
    def inner(*args, **kwargs):
        ...
        for key in elts_dict.keys():
            if is_symbol(key):
                ...
                raise ValueError(
                    "Input has to be either atomic number, either chemical symbols"
                )
...
def quant_spectrum(s1, skip_elements=[]):
    ...
    selected_elements = [
        element for element in selected_elements if element not in skip_elements
    ]
...
def cluster_analysis_concentration_report(s, cluster_source=None, print_std=False):
    ...
    for el, q, d in zip(els, qs, ds):
        print(el, end="\t")
        if print_std:
            q_d = [str(qi) + "\u00b1" + str(di) for qi, di in zip(q, d)]
            print(*q_d, sep="\t")
...
def fancy_cluster_plot(s):
    ...
    n = data.shape[0]
    d = (
        (data * np.array(range(1, n + 1))[:, np.newaxis, np.newaxis])
        .sum(0)
        .astype("float")
    )
    d[d == 0] = np.nan
...
def quant_profile_linear_fit(pf, **linfit_kwargs):
    ...
    for i, p in enumerate(pf):
        ...
        out = lr.fit(xf[keep].reshape(-1, 1), yf[keep].reshape(-1, 1))
    ...
    if out.ndim > 2:
        for i, m in enumerate(out):
            if erosion_radius > 0 and extra_safe:
                m = ski.morphology.binary_erosion(
                    m, ski.morphology.disk(erosion_radius)
                )
