...
class SmoothNMF(NMFEstimator):
    ...
    **kwargs : dict
    ...
    loss_names_ = (
        NMFEstimator.loss_names_ + ["log_reg_loss"] + ["Lapl_reg_loss"] + ["gamma"]
    )
    ...
    def check_params(self):
        ...
        if self.algo not in [
            ...
            print(
                "The algorithm must be 'l2_surrogate', 'log_surrogate', 'bmd' or 'projected_gradient'"
            )
            print("The algorithm is set to 'log_surrogate'")
            self.algo = "log_surrogate"
        ...
        if not (np.all(np.array(self.mu) >= 0)):
            ...
            self.mu = 0
        if not (
            (self.simplex_H and not (self.simplex_W))
            or (not (self.simplex_H) and self.simplex_W)
            or (not (self.simplex_H) and not (self.simplex_W))
        ):
            print(
                "The simplex constraint must be applied to either W or H or none of them"
            )
        ...
        if self.linesearch:
            ...
            if not (self.lambda_L > 0):
                print(
                    "The regularization parameter lambda_L must be non-zero when using linesearch"
                )
