...
def absorption_mass_thickness(
    x, mass_thickness, toa=90, atomic_fraction=True, *, elements_dict={"Si": 1.0}
):
    r"""
    ...
    if type(mu) == float:
        ...
        chi = (
            mu[:, np.newaxis]
            @ ((mass_thickness.reshape(-1)[:, np.newaxis]).T)
            / np.sin(rad_toa)
        )
    return (1 - np.exp(-chi)) / chi
