...
def sample_to_EDSespm(sample, elements=[]):
    ...
    s.metadata.Acquisition_instrument.TEM.Detector.EDS.type = model_params[
        "params_dict"
    ]["Det"]
    s.metadata.Acquisition_instrument.TEM.Detector.EDS.take_off_angle = model_params[
        "params_dict"
    ]["Abs"]["toa"]
    s.metadata.Acquisition_instrument.TEM.Detector.EDS.width_slope = model_params[
        "width_slope"
    ]
    s.metadata.Acquisition_instrument.TEM.Detector.EDS.width_intercept = model_params[
        "width_intercept"
    ]
...
def generate_spim_sample(
    phases, weights, model_params, misc_params, seed=0, g_params={}
):
    r"""
    ...
    assert np.allclose(np.sum(weights, axis=2), 1.0), (
        "The input weights do not sum to one. Please modify it so that they sum to one along axis 2"
    )
    Xdot = generate_spim(
        phases,
        weights,
        misc_params["densities"],
        misc_params["N"],
        seed=seed,
        continuous=True,
    )
...
def generate_dataset(
    *args, base_path=DATASETS_PATH, sample_number=10, base_seed=0, elements=[], **kwargs
):
    r"""
    ...
    for i in tqdm(range(sample_number)):
        ...
        if sample["misc_parameters"]["model"] == "EDXS":
            ...
            raise ValueError(
                "Unknown model. The implemented models are 'EDXS' and 'Toy"
            )
        hs_sig.save(
            base_path
            / Path(sample["misc_parameters"]["data_folder"])
            / Path(f"sample_{i}.hspy")
        )
