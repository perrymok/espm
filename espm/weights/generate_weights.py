...
def gaussian_ripple_weights(shape_2d, width=1, seed=0, **kwargs):
    ...
    if seed == 0:
        a.add_gaussian_ripple(
            center=shape_2d[1] // 2, width=width, conc_max=1, phase_id=1
        )
    else:
        np.random.seed(seed)
        a.add_gaussian_ripple(
            center=np.random.randint(1, shape_2d[1]),
            width=width,
            conc_max=1,
            phase_id=1,
        )
...
def spheres_weights(shape_2d=[80, 80], n_phases=3, seed=0, radius=1, **kwargs):
    ...
    for i in range(1, n_phases):
        a.add_sphere(
            (np.random.randint(1, shape_2d[0]), np.random.randint(1, shape_2d[1])),
            radius,
            1 / (n_phases - 1),
            i,
        )
...
def chemical_map_weights(file=None, line_list=[], conc_list=[], **kwargs):
    ...
    if file is None or line_list == []:
        print(
            "Please provide a file name and a list of X-ray emission lines. Nothing was done."
        )
    else:
        ...
        for i in range(len(line_list)):
            a.add_chemical_map(
                file, line_list[i], 0.0, conc_list[i], sigma=3, phase_id=1 + i
            )
